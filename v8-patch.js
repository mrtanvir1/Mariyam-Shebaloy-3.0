...
(function () {
  ...
  function renderPrintPage8(p,v){
    ...
    const follow=(v.followupDays||v.followupDate) ? `${v.followupDays?`${esc8(v.followupDays)} দিন পর`:""}${v.followupDate?` — ${esc8(v.followupDate)}`:""}` : "-";
    const dateText=new Date(v.date).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"});
    return `<div id="v8PrintPage" class="rx-sheet">
      <div class="rx-topbar"></div>
      <div class="rx-header">
        <div class="rx-doctor">
          <div class="rx-doctor-name">${esc8(pr.name||"")}</div>
          <div class="rx-degree">${esc8(pr.degree||"")}</div>
          <div class="rx-contact">${esc8(phones)}</div>
          <div class="rx-address">${esc8(pr.address||"")}</div>
        </div>
        <div class="rx-clinic">
      ...
      <div class="rx-patient-title">PATIENT INFORMATION</div>
      <div class="rx-patient-grid">
        <div><span>নাম</span><b>${esc8(p.name||"-")}</b></div>
        <div><span>বয়স</span><b>${esc8(p.age||"-")}</b></div>
      ...
      <div class="rx-main">
        <aside class="rx-sidebar">
          <section class="rx-side-card">
            <div class="rx-side-head">CC – Chief Complaints</div>
            <div class="rx-side-body">${esc8(cc).replace(/\n/g,"<br>")}</div>
          </section>

          <section class="rx-side-card">
            <div class="rx-side-head">O/E – On Examination</div>
            <div class="rx-vitals">
          ...
          <section class="rx-side-card">
            <div class="rx-side-head">Diagnosis</div>
            <div class="rx-side-body">${esc8(diagnosis).replace(/\n/g,"<br>")}</div>
          </section>

          <section class="rx-side-card">
            <div class="rx-side-head">Investigation</div>
            <div class="rx-side-body">${inv.length ? inv.map(x=>`• ${esc8(x)}`).join("<br>") : "-"}</div>
          </section>

          <section class="rx-side-card">
            <div class="rx-side-head">Advice</div>
            <div class="rx-side-body">${esc8(advice).replace(/\n/g,"<br>")}</div>
          </section>

          <section class="rx-side-card">
            <div class="rx-side-head">Follow-up</div>
            <div class="rx-side-body">${follow}</div>
          </section>
        </aside>

        <main class="rx-prescription">
          <div class="rx-symbol">℞</div>
          <div class="rx-prescription-title">PRESCRIPTION / MEDICINES</div>
          ${meds.length ? `
          <table class="rx-table">
            ...
            <tbody>${meds.map((m,i)=>`
              <tr>
                ...
                <td><b>${esc8(m.name||"")} ${esc8(m.strength||"")}</b>${m.generic||m.form?`<br><span class="rx-sub">${esc8(m.generic||"")}${m.generic&&m.form?"<br>":""}${esc8(m.form||"")}</span>`:""}</td>
                <td>${esc8(m.frequency||"-")}</td>
                <td>${esc8(translateFood8(m.food))}</td>
                <td>${esc8(m.duration||"-")}</td>
              </tr>`).join("")}</tbody>
          </table>` : `<div class="rx-empty">No medicine added</div>`}

          <div class="rx-signature">
            <div class="rx-sign-line"></div>
            <div><b>${esc8(pr.name||"")}</b></div>
            <div>${esc8(pr.degree||"")}</div>
          </div>
        </main>
      </div>

      <div class="rx-footer">
        <span>${esc8(pr.clinic||"")} · Prescription</span>
        <span>Patient ID: ${esc8(p.id||"-")}</span>
      </div>
    </div>`;
  }
  ...
  window.printPrescription=function(patientId,visitId){
    ...
    const style=document.createElement("style"); style.id="v8PrintStyle"; style.textContent=`@page{size:A4 portrait;margin:0}html,body{margin:0;padding:0;background:#fff}.rx-sheet{box-sizing:border-box;width:210mm;min-height:297mm;padding:7mm 14mm 8mm;background:#fff;color:#193b3d;font-family:Arial,'Noto Sans Bengali','Noto Sans',sans-serif;font-size:12px}.rx-topbar{height:3px;background:#008c83;margin:0 -2mm 2mm}.rx-header{height:27mm;box-sizing:border-box;background:#eefbfb;border:1px solid #cfe7e6;padding:5mm 5mm 4mm;display:flex;justify-content:space-between;border-top:0}.rx-doctor-name{font-size:24px;line-height:1.05;font-weight:800;color:#075d5b;letter-spacing:.1px}.rx-degree{font-size:13px;font-weight:700;color:#42595a;margin-top:2px}.rx-contact,.rx-address{font-size:11px;color:#516263;line-height:1.45}.rx-clinic{text-align:right;padding-top:1mm}.rx-clinic-name{font-size:16px;font-weight:800;color:#087f7a}.rx-date{font-size:11px;color:#687778;margin-top:4px}.rx-patient-title{margin-top:3mm;height:7mm;box-sizing:border-box;padding:2mm 3mm 1mm;background:#e9f9f9;border:1px solid #cfe7e6;border-bottom:0;border-radius:3mm 3mm 0 0;font-size:11px;font-weight:800;color:#086b68}.rx-patient-grid{display:grid;grid-template-columns:2fr .55fr .7fr 1.05fr 1.05fr;border:1px solid #cfe1e1;border-radius:0 0 3mm 3mm;overflow:hidden;height:14mm}.rx-patient-grid>div{padding:2mm 2.5mm;display:flex;flex-direction:column;justify-content:center;border-right:1px solid #d6e5e5}.rx-patient-grid>div:last-child{border-right:0}.rx-patient-grid span{font-size:9px;color:#657677;margin-bottom:1mm}.rx-patient-grid b{font-size:12px;color:#334748}.rx-main{display:grid;grid-template-columns:49mm 1fr;gap:4mm;margin-top:3mm;min-height:205mm}.rx-sidebar{border-right:1px solid #d3e3e2;padding-right:3mm}.rx-side-card{border:1px solid #cfe3e2;border-radius:3mm;overflow:hidden;margin-bottom:3mm;background:#fff;break-inside:avoid}.rx-side-head{height:7mm;box-sizing:border-box;padding:1.8mm 2.5mm;background:#e9f9f9;border-bottom:1px solid #cfe3e2;font-size:11px;font-weight:800;color:#08706d}.rx-side-body{min-height:9mm;padding:3mm 2.5mm;color:#4b5c5d;font-size:11px;line-height:1.45}.rx-vitals{padding:3mm;display:grid;grid-template-columns:1fr 1fr;gap:2mm}.rx-vitals>div{border:1px solid #d9e6e5;border-radius:2.5mm;padding:2mm;min-height:10mm;display:flex;flex-direction:column;justify-content:center}.rx-vitals span{font-size:9px;color:#08706d;font-weight:700}.rx-vitals b{font-size:11px;color:#4a5b5c;margin-top:1mm}.rx-prescription{position:relative;padding-top:1mm;min-width:0}.rx-symbol{font-family:Georgia,serif;font-size:32px;font-weight:700;line-height:1;color:#007d78;margin:0 0 2mm}.rx-prescription-title{font-size:11px;color:#6c7a7a;margin-bottom:2mm}.rx-table{width:100%;border-collapse:collapse;table-layout:fixed;border:1px solid #c9dbda}.rx-table th{background:#008b82;color:#fff;font-size:10px;font-weight:800;padding:2.2mm 2mm;text-align:left;border-right:1px solid rgba(255,255,255,.25)}.rx-table th:nth-child(1){width:7%}.rx-table th:nth-child(2){width:47%}.rx-table th:nth-child(3){width:14%}.rx-table th:nth-child(4){width:16%}.rx-table th:nth-child(5){width:16%}.rx-table td{border:1px solid #d5e2e1;padding:2.5mm 2mm;font-size:10.5px;vertical-align:top;color:#334849}.rx-num{text-align:center}.rx-sub{font-size:9px;color:#687777;line-height:1.35}.rx-empty{border:1px solid #d5e2e1;padding:4mm;color:#687777}.rx-signature{text-align:right;margin-top:9cm;padding-right:2mm;color:#596a6a;font-size:10px;line-height:1.45}.rx-sign-line{width:43mm;border-top:1px solid #596a6a;margin-left:auto;margin-bottom:2mm}.rx-signature b{font-size:11px;color:#4a595a}.rx-footer{position:relative;margin-top:12mm;border-top:1px solid #cfe1e0;padding-top:2mm;display:flex;justify-content:space-between;color:#6b7a7a;font-size:9px}.rx-footer span:first-child{color:#1b7370}@media screen{#v8PrintOverlay{position:fixed;inset:0;z-index:20000;background:#fff;overflow:auto;padding:20px;box-sizing:border-box}#v8PrintOverlay:before{content:'🖨 Print Preview';display:block;background:#008b82;color:#fff;padding:10px;font-weight:700;text-align:center;max-width:210mm;margin:0 auto 12px;border-radius:4px}.rx-sheet{margin:0 auto;box-shadow:0 2px 18px rgba(0,0,0,.12)}}@media print{body>*:not(#v8PrintOverlay){display:none!important}#v8PrintOverlay{display:block!important;position:static!important;background:#fff!important;padding:0!important}#v8PrintOverlay:before{display:none!important}.rx-sheet{box-shadow:none!important;width:210mm!important;min-height:297mm!important;padding:7mm 14mm 8mm!important}.rx-sidebar{border-right:1px solid #d3e3e2!important}.rx-main{grid-template-columns:49mm 1fr!important}}@media(max-width:600px){.rx-patient-grid{grid-template-columns:1.6fr .6fr .7fr 1fr 1fr}.rx-main{grid-template-columns:1fr}.rx-sidebar{border-right:0;padding-right:0}.rx-signature{margin-top:60mm}}`;
